...
open class Highlighter {
    ...
    */
    /// Locates `Highlighter_Highlighter.bundle` (the SwiftPM resource bundle
    /// for this target) without touching the generated `Bundle.module`
    /// accessor, which traps when its two hardcoded candidates are absent.
    private static let resourceBundle: Bundle? = {
        let bundleName = "Highlighter_Highlighter.bundle"
        var candidates: [URL] = []
        // A packaged .app: package.sh copies the bundle into
        // Contents/Resources, which is the main bundle's resource URL.
        if let resources = Bundle.main.resourceURL {
            candidates.append(resources)
        }
        // `swift build` / `swift run`: resource bundles sit next to the
        // executable, which is what the main bundle's URL points at for a
        // bare binary.
    ...
    public init?() {
        ...
        //
        // VENDORED CHANGE (diffreview): do NOT use SwiftPM's generated
        // `Bundle.module`. The `swift build` variant of that accessor only
        // checks the app-bundle ROOT and an absolute path baked in at compile
        // time (the builder's .build directory) — and it calls fatalError()
        // when both miss. DiffReview.app ships this bundle in
        // Contents/Resources (see package.sh), so 0.1.0 crashed on the first
        // highlight once the original build directory was gone. Resolve the
        // bundle from the places the app actually uses, and fail soft:
        // returning nil degrades to plain-text rendering instead of killing
        // the app.
        guard let bundle = Highlighter.resourceBundle else {
            return nil
        }
