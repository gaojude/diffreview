...
struct WelcomeView: View {
        ...
        VStack(spacing: 28) {
            ...
            HStack(spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 5) {
                    Text("Use Redline from Terminal")
                        .font(.headline)
                    Text(cliDescription)
    ...
    private var cliAction: some View {
        ...
        case .available(let existingCommand):
            ...
            .accessibilityIdentifier("install-redline-cli")
        case .installed:
    ...
    private var cliDescription: String {
        ...
        case .available(let existingCommand):
            return existingCommand
                ? "Replace the existing /usr/local/bin/redline command with this copy."
                : "Install the redline command in /usr/local/bin so it’s available from any project."
        case .installed:
            return "The redline command is installed and ready to use."
        case .installing:
            return "Installing /usr/local/bin/redline…"
